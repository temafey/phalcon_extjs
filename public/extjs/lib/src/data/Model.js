...
Ext.define('Ext.data.Model', {
    ...
    set: function (fieldName, newValue) {
        var me = this,
            ...
            currentValue, field, idChanged, key, modifiedField\Names, name, oldId,
            newId, value, values;
        ...
        for (name in values) {
            if (values.hasOwnProperty(name)) {
                ...
                data[name] = value;
                (modifiedField\Names || (modifiedField\Names = [])).push(name);
        ...
        if (!me.editing && modifiedField\Names) {
            me.afterEdit(modifiedField\Names);
        }

        return modifiedField\Names || null;
    },
     ...
     * @param {String[]} [modifiedField\Names] Array of field names changed during edit.
     */
    endEdit : function(silent, modifiedField\Names){
        var me = this,
            dataSave,
            changed;
        ...
        if (me.editing) {
            ...
            me.modifiedSave = me.dataSave = me.dirtySave = null;
            if (!silent) {
                if (!modifiedField\Names) {
                    modifiedField\Names = me.getModifiedField\Names(dataSave);
                }
                changed = me.dirty || modifiedField\Names.length > 0;
                if (changed) {
                    me.afterEdit(modifiedField\Names);
                }
            }
     ...
     */
    getModifiedField\Names: function(saved){
        var me = this,
            data = me[me.persistenceProperty],
            modified = [],
            key;
     ...
     * @param {String[]} [modifiedField\Names] Array of field names changed during sync with server if known.
     * Omit or pass `null` if unknown. An empty array means that it is known that no fields were modified
     * by the server's response.
     * Defaults to false.
     */
    commit : function(silent, modifiedField\Names) {
        var me = this;
        ...
        if (silent !== true) {
            me.afterCommit(modifiedField\Names);
        }
    },
     ...
     * @param {String[]} [modifiedField\Names] Array of field names changed during edit.
     */
    afterEdit : function(modifiedField\Names) {
        this.callStore('afterEdit', modifiedField\Names);
    },
     ...
     * @param {String[]} [modifiedField\Names] Array of field names changed by syncing this field with the server.
     */
    afterCommit: function(modifiedField\Names) {
        this.callStore('afterCommit', modifiedField\Names);
    },
