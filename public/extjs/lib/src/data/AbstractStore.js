...
Ext.define('Ext.data.AbstractStore', {
     ...
     * @since 3.4.0
     */
    afterEdit : function(record, modifiedField\Names) {
        var me = this,
            i, shouldSync;

        if (me.autoSync && !me.autoSyncSuspended) {
            for (i = modifiedField\Names.length; i--;) {
                // only sync if persistent fields were modified
                if (record.fields.get(modifiedField\Names[i]).persist) {
                    shouldSync = true;
                    break;
                }
            }
        ...
        me.onUpdate(record, Ext.data.Model.EDIT, modifiedField\Names);
        me.fireEvent('update', me, record, Ext.data.Model.EDIT, modifiedField\Names);
    },
    ...
    afterReject : function(record) {
        // Must pass the 5th param (modifiedField\Names) as null, otherwise the
     ...
     * @since 3.4.0
     */
    afterCommit : function(record, modifiedField\Names) {
        if (!modifiedField\Names) {
            modifiedField\Names = null;
        }
        this.onUpdate(record, Ext.data.Model.COMMIT, modifiedField\Names);
        this.fireEvent('update', this, record, Ext.data.Model.COMMIT, modifiedField\Names);
    },
