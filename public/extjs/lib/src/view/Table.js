...
Ext.define('Ext.view.Table', {
    ...
    */
    ...
    // private
    onUpdate : function(store, record, operation, changedField\Names) {
        var me = this,
            rowTpl = me.rowTpl,
            index,
            oldRow, oldRowDom,
        ...
        if (me.viewReady) {
            ...
            if (oldRowDom) {
                ...
                if (columns.length) {
                    me.updateColumns(record, me.getNode(oldRowDom, true), me.getNode(newRowDom, true), columns, changedField\Names);
                }
    ...
    updateColumns: function(record, oldRowDom, newRowDom, columns, changedField\Names) {
        var me = this,
            newAttrs, attLen, attName, attrIndex,
        ...
        for (colIndex = 0; colIndex < colCount; colIndex++) {
            ...
            if (me.shouldUpdateCell(record, column, changedField\Names)) {
    ...
    shouldUpdateCell: function(record, column, changedField\Names){
        ...
        // If no changeField\Names array was passed, we have to assume that that information
        // is unknown and update all cells.
        if (column.hasCustomRenderer || !changedField\Names) {
            return true;
        }

        if (changedField\Names) {
            var len = changedField\Names.length,
                i, field;

            for (i = 0; i < len; ++i) {
                field = changedField\Names[i];
